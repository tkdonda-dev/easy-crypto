/**
 * Easy Crypto - Complete Node.js Universal Demo
 * Demonstrates all features with real-world use cases (async universal API)
 * 
...
(async () => {
  ...
  const encryptedObj = await crypto.encryptObject(userData, password);
  console.log(`\nEncrypted: ${encryptedObj.substring(0, 60)}...\n`);

  const decryptedObj = await crypto.decryptObject(encryptedObj, password);
  ...
  const hashedPassword = await crypto.hash(userPassword);
  ...
  const isValid = await crypto.verifyHash(loginAttempt1, hashedPassword);
  ...
  const isInvalid = await crypto.verifyHash(loginAttempt2, hashedPassword);
  ...
  const { key: key2 } = await crypto.generateKey(masterPassword, salt);
  ...
  const encryptedBase64 = await crypto.encrypt(textToEncrypt, password, { output: 'base64' });
  ...
  // Raw Uint8Array output
  const encryptedRaw = await crypto.encrypt(textToEncrypt, password, { output: 'raw' });
  ...
  const decryptedFromBase64 = await crypto.decrypt(encryptedBase64, password);
  const decryptedFromRaw = await crypto.decrypt(encryptedRaw, password, { input: 'raw' });
  ...
  try {
    await crypto.encrypt('test', 'short');
  } catch (error) {
  ...
  try {
    const enc = await crypto.encrypt('secret', 'correct-password');
    await crypto.decrypt(enc, 'wrong-password');
  } catch (error) {
  ...
  try {
    await crypto.decrypt('invalid-base64-data', password);
  } catch (error) {
  ...
  const encryptedKeys = await crypto.encryptObject(apiKeys, 'master-encryption-key-2024');
  console.log(`\nEncrypted for database storage:`);
  console.log(encryptedKeys.substring(0, 80) + '...\n');

  const decryptedKeys = await crypto.decryptObject(encryptedKeys, 'master-encryption-key-2024');
